...
def emnist_train(model):
    ...
    model.fit(X_train, x_train_cat, validation_data=(X_test, y_test_cat), callbacks=[learning_rate_reduction], batch_size=64, epochs=600)
...
def emnist_predict_img(model, img):
    ...
    img_arr = np.concatenate(img)
    img_arr = img_arr / 255.0
    img_arr = np.reshape(img_arr, (4096,))

    img_tensor = tf.convert_to_tensor([img_arr])
    result = np.argmax(model.predict(img_tensor, batch_size=1), axis=-1)
    res = ch.get_chars()[result[0]]
    return res
...
def img_to_str(model: Any, image_file: str):
    letters = letters_extract(image_file, out_size=64)
    s_out = ""
...
if __name__ == "__main__":
    ...
    s_out = img_to_str(model, "img_1.png")
    print(s_out)