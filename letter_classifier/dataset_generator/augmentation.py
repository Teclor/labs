...
class Augmentation:
    ...
    def __init__(
        ...
        self.letters_initial_dir = os.path.relpath(self.settings.get('DATASET_PATH') + '_initial' if letters_initial_dir is None else letters_initial_dir)
        self.letters_dataset_dir = self.settings.get('DATASET_PATH') if letters_dataset_dir is None else letters_initial_dir
        ...
        self.out_size = self.settings.get('IMAGE_SIZE')
    ...
    def __len__(self):
        return len(self.v_shift_values) * len(self.h_shift_values) * len(self.rotations) * len(self.blur_values) * len(
            self.letter_size_list) * len(ch.get_chars()) * len(self.settings.get('FONT_LIST'))
